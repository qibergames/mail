...
export function AdminApp({ section }: { section: AdminSection }) {
  ...
  async function post(body: unknown, form?: HTMLFormElement) {
    ...
    setBusy(false)
    if (response.ok) { form?.reset(); setCreateOpen(false); setEditingRule(null); setEditingMailbox(null); await load() }
  }
  ...
  return <>
    ...
    <AdminModal open={createOpen} title={i18n._(editingRule ? 'Edit rule' : editingMailbox ? 'Edit mailbox' : details.action || details.title)} onClose={() => { setCreateOpen(false); setEditingRule(null); setEditingMailbox(null); setStatus(null) }} status={status} description={editingRule ? 'Changes apply to new incoming mail immediately.' : editingMailbox ? 'The address itself cannot change; create an alias for a new address.' : details.hint}>
      ...
      {section === 'mailboxes' && <form key={editingMailbox?.id ?? 'new'} className="grid gap-4 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); void post(editingMailbox ? { action: 'mailbox:update', mailboxId: editingMailbox.id, userId: form.get('userId'), displayName: form.get('displayName'), mailboxType, disabled: form.get('disabled') === 'on' } : { action: 'mailbox:create', userId: form.get('userId'), domainId: form.get('domainId'), localPart: form.get('localPart'), displayName: form.get('displayName'), mailboxType }, event.currentTarget) }}>
        ...
        {editingMailbox
          ? <><Field label="Address" name="addressLabel" value={`${editingMailbox.localPart}@${editingMailbox.hostname}`} readOnly className="sm:col-span-2" /><CheckboxField label="Disabled" name="disabled" defaultChecked={editingMailbox.disabled} className="sm:col-span-2" /></>
        ...
        <FormActions busy={busy || (!editingMailbox && !data.domains.length)} close={() => { setCreateOpen(false); setEditingMailbox(null) }} label={editingMailbox ? 'Save mailbox' : 'Create mailbox'} />
      </form>}
      ...
      {section === 'routing' && <form key={editingRule?.id ?? 'new'} className="grid gap-4 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); const catchAll = ruleMode === 'catch_all'; const body = { mailboxId: form.get('mailboxId') || null, name: form.get('name') || (catchAll ? 'Catch-all' : form.get('pattern')), pattern: catchAll ? '*' : form.get('pattern'), actionType: ruleAction, forwardTo: form.get('forwardTo') ?? '', keepCopy: form.get('keepCopy') === 'on' }; void post(editingRule ? { action: 'rule:update', ruleId: editingRule.id, enabled: form.get('enabled') === 'on', ...body } : { action: 'rule:create', domainId: form.get('domainId'), ...body }, event.currentTarget) }}>
        ...
        <FormActions busy={busy || (!editingRule && !data.domains.length) || (ruleAction === 'store' && !data.mailboxes.length)} close={() => { setCreateOpen(false); setEditingRule(null) }} label={editingRule ? 'Save rule' : 'Add rule'} />
      </form>}
    </AdminModal>
  </>
}
