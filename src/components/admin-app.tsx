...
export function AdminApp({ section }: { section: AdminSection }) {
  ...
  const [editingRule, setEditingRule] = useState<AdminData['rules'][number] | null>(null)
  const [editingMailbox, setEditingMailbox] = useState<AdminData['mailboxes'][number] | null>(null)
  const [editingAlias, setEditingAlias] = useState<AdminData['aliases'][number] | null>(null)
  const [editingAccess, setEditingAccess] = useState<AdminData['access'][number] | null>(null)
  ...
  function closeEditors() { setEditingRule(null); setEditingMailbox(null); setEditingAlias(null); setEditingAccess(null) }
  ...
  async function post(body: unknown, form?: HTMLFormElement) {
    ...
    setBusy(false)
    if (response.ok) { form?.reset(); setCreateOpen(false); closeEditors(); await load() }
  }
  ...
  return <>
    ...
    <section className="space-y-4">
      ...
      {section === 'mailboxes' && <ItemGrid>{data.mailboxes.map((mailbox) => <ItemCard key={mailbox.id} icon={Mail} title={`${mailbox.localPart}@${mailbox.hostname}`} description={`${mailbox.displayName ? `${mailbox.displayName} · ` : ''}${data.users.find((user) => user.id === mailbox.userId)?.name ?? '—'}`} badges={<><Badge>{mailbox.type}</Badge>{mailbox.disabled && <Badge danger><Trans id="Disabled" /></Badge>}</>} actions={<><Button size="icon-sm" variant="ghost" disabled={busy} onClick={() => { setEditingMailbox(mailbox); setMailboxType(mailbox.type); setCreateOpen(true) }} aria-label={i18n._('Edit mailbox')} title={i18n._('Edit mailbox')}><Pencil /></Button><Button size="icon-sm" variant="ghost" className="hover:bg-red-500/10 hover:text-red-600" disabled={busy} onClick={() => { if (confirm(i18n._('Delete this mailbox? Its aliases and routing are removed and its messages become inaccessible.'))) void post({ action: 'mailbox:delete', mailboxId: mailbox.id }) }} aria-label={i18n._('Delete mailbox')} title={i18n._('Delete mailbox')}><Trash2 /></Button></>} />)}</ItemGrid>}
    ...
    <AdminModal open={createOpen} title={i18n._(editingRule ? 'Edit rule' : editingMailbox ? 'Edit mailbox' : editingAlias ? 'Edit alias' : editingAccess ? 'Edit access' : details.action || details.title)} onClose={() => { setCreateOpen(false); closeEditors(); setStatus(null) }} status={status} description={editingRule ? 'Changes apply to new incoming mail immediately.' : editingMailbox ? 'Update the address, owner or type of this mailbox.' : editingAlias ? 'Change the alias address or where it delivers.' : editingAccess ? 'Change what this account may do with the mailbox.' : details.hint}>
      ...
      {section === 'mailboxes' && <form key={editingMailbox?.id ?? 'new'} className="grid gap-4 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); void post(editingMailbox ? { action: 'mailbox:update', mailboxId: editingMailbox.id, userId: form.get('userId'), domainId: form.get('domainId'), localPart: form.get('localPart'), displayName: form.get('displayName'), mailboxType, disabled: form.get('disabled') === 'on' } : { action: 'mailbox:create', userId: form.get('userId'), domainId: form.get('domainId'), localPart: form.get('localPart'), displayName: form.get('displayName'), mailboxType }, event.currentTarget) }}>
        ...
        <SelectField label={mailboxType === 'shared' ? 'Owner' : 'Account'} name="userId" defaultValue={editingMailbox?.userId} options={data.users.map((user) => [user.id, user.name] as [string, string])} />
        <Field label="Display name" name="displayName" defaultValue={editingMailbox?.displayName ?? ''} placeholder={mailboxType === 'shared' ? i18n._('Support team') : ''} />
        <div className="grid grid-cols-[1fr_auto_1fr] items-end gap-2 sm:col-span-2"><Field label="Address" name="localPart" defaultValue={editingMailbox?.localPart ?? ''} placeholder={mailboxType === 'shared' ? 'support' : 'name'} required /><span className="pb-2.5 text-muted-foreground">@</span><SelectField label="Domain" name="domainId" defaultValue={editingMailbox?.domainId} options={data.domains.map((domain) => [domain.id, domain.hostname] as [string, string])} /></div>
        {editingMailbox && <><InfoNote><Trans id="Changing the address moves the Cloudflare route to the new address. If the owner signs in with the old address, the sign-in email follows." /></InfoNote><CheckboxField label="Disabled" name="disabled" defaultChecked={editingMailbox.disabled} className="sm:col-span-2" /></>}
        <FormActions busy={busy || (!editingMailbox && !data.domains.length)} close={() => { setCreateOpen(false); closeEditors() }} label={editingMailbox ? 'Save mailbox' : 'Create mailbox'} />
      </form>}
      {section === 'aliases' && <form key={editingAlias?.id ?? 'new'} className="grid gap-4" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); const body = { mailboxId: form.get('mailboxId'), domainId: form.get('domainId'), localPart: form.get('localPart') }; void post(editingAlias ? { action: 'alias:update', aliasId: editingAlias.id, ...body } : { action: 'alias:create', ...body }, event.currentTarget) }}>
        <div className="grid grid-cols-[1fr_auto_1fr] items-end gap-2"><Field label="Alias address" name="localPart" placeholder="hello" defaultValue={editingAlias?.localPart ?? ''} required /><span className="pb-2.5 text-muted-foreground">@</span><SelectField label="Domain" name="domainId" defaultValue={editingAlias?.domainId} options={data.domains.map((domain) => [domain.id, domain.hostname] as [string, string])} /></div>
        <SelectField label="Deliver to mailbox" name="mailboxId" defaultValue={editingAlias?.mailboxId} options={data.mailboxes.map((mailbox) => [mailbox.id, mailboxAddress(data, mailbox.id)] as [string, string])} />
        <FormActions busy={busy || !data.mailboxes.length} close={() => { setCreateOpen(false); closeEditors() }} label={editingAlias ? 'Save alias' : 'Add alias'} />
      </form>}
      {section === 'access' && <form key={editingAccess?.id ?? 'new'} className="grid gap-4 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); void post({ action: 'access:set', mailboxId: form.get('mailboxId'), userId: form.get('userId'), permission: form.get('permission') }, event.currentTarget) }}><SelectField label="Mailbox" name="mailboxId" defaultValue={editingAccess?.mailboxId} disabled={Boolean(editingAccess)} options={data.mailboxes.map((mailbox) => [mailbox.id, mailboxAddress(data, mailbox.id)] as [string, string])} /><SelectField label="Account" name="userId" defaultValue={editingAccess?.userId} disabled={Boolean(editingAccess)} options={data.users.map((user) => [user.id, user.name] as [string, string])} />{editingAccess && <><input type="hidden" name="mailboxId" value={editingAccess.mailboxId} /><input type="hidden" name="userId" value={editingAccess.userId} /></>}<SelectField label="Permission" name="permission" defaultValue={editingAccess?.permission} options={['read_only', 'send_as', 'send_on_behalf', 'full_access']} /><FormActions busy={busy || !data.mailboxes.length} close={() => { setCreateOpen(false); closeEditors() }} label="Save access" /></form>}
      {section === 'routing' && <form key={editingRule?.id ?? 'new'} className="grid gap-4 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); const catchAll = ruleMode === 'catch_all'; const body = { mailboxId: form.get('mailboxId') || null, name: form.get('name') || (catchAll ? 'Catch-all' : form.get('pattern')), pattern: catchAll ? '*' : form.get('pattern'), actionType: ruleAction, forwardTo: form.get('forwardTo') ?? '', keepCopy: form.get('keepCopy') === 'on' }; void post(editingRule ? { action: 'rule:update', ruleId: editingRule.id, enabled: form.get('enabled') === 'on', ...body } : { action: 'rule:create', domainId: form.get('domainId'), ...body }, event.currentTarget) }}>
        {editingRule
          ? <Field label="Domain" name="domainLabel" value={data.domains.find((domain) => domain.id === editingRule.domainId)?.hostname ?? ''} readOnly />
        ...
        {ruleMode === 'pattern'
          ? <Field label="Recipient pattern" name="pattern" placeholder="sales" defaultValue={editingRule && editingRule.pattern !== '*' ? editingRule.pattern : ''} required className="sm:col-span-2" />
        ...
        {ruleAction === 'store' && <SelectField label="Deliver to mailbox" name="mailboxId" required defaultValue={editingRule?.mailboxId ?? undefined} options={data.mailboxes.map((mailbox) => [mailbox.id, mailboxAddress(data, mailbox.id)] as [string, string])} />}
        {ruleAction === 'forward' && <><Field label="Forward to" name="forwardTo" type="email" defaultValue={editingRule?.forwardTo ?? ''} required /><SelectField label="Keep a copy in" name="mailboxId" defaultValue={editingRule?.mailboxId ?? ''} options={[['', i18n._('No mailbox')], ...data.mailboxes.map((mailbox) => [mailbox.id, mailboxAddress(data, mailbox.id)] as [string, string])]} /><CheckboxField label="Keep a copy" name="keepCopy" defaultChecked={editingRule ? editingRule.keepCopy : true} /></>}
        {ruleAction === 'reject' && <InfoNote><Trans id="The sender receives a bounce and nothing is stored." /></InfoNote>}
        <Field label="Rule name" name="name" placeholder={ruleMode === 'catch_all' ? 'Catch-all' : ''} defaultValue={editingRule?.name ?? ''} className="sm:col-span-2" />
        {editingRule && <CheckboxField label="Rule enabled" name="enabled" defaultChecked={editingRule.enabled} className="sm:col-span-2" />}
        <FormActions busy={busy || (!editingRule && !data.domains.length) || (ruleAction === 'store' && !data.mailboxes.length)} close={() => { setCreateOpen(false); closeEditors() }} label={editingRule ? 'Save rule' : 'Add rule'} />
      </form>}
    </AdminModal>
  </>
}
