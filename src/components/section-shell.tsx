    ...
    <div className="mx-auto max-w-7xl space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-[15rem_minmax(0,1fr)]">
        <aside className="min-w-0 rounded-xl border bg-background p-2 lg:sticky lg:top-6 lg:h-fit lg:p-3">
          <div className="flex gap-4 overflow-x-auto lg:grid lg:gap-5">
            {groups.map((group) => <section key={group} className="shrink-0 lg:shrink">
              ...
              <nav className="flex gap-1 lg:grid" aria-label={i18n._(group)}>
...
function SectionLink({ area, item }: { area: 'settings' | 'tools' | 'admin'; item: SectionNavItem }) {
  ...
  const props = { className: 'flex h-9 items-center gap-2 whitespace-nowrap rounded-lg px-3 text-sm font-medium text-muted-foreground hover:bg-muted hover:text-foreground [&.active]:bg-secondary [&.active]:text-foreground', activeOptions: { exact: true } }
