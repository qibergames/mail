...
import { CalendarClock, ChevronDown, FileText, TriangleAlert, LoaderCircle, Maximize2, Minimize2, Minus, Paperclip, Send, Trash2, X } from 'lucide-react'
...
export function Composer({ mailboxes, mailboxId, draft, close, sent }: { mailboxes: Array<ComposerMailbox>; mailboxId: string; draft: Draft; close: () => void; sent: () => void }) {
  ...
  const [undeliverable, setUndeliverable] = useState<Map<string, string>>(new Map())
  ...
  useEffect(() => {
    void fetch('/api/tools').then((response) => response.json<{ templates: Array<Template>; contacts: Array<{ email: string; undeliverableAt: string | null; undeliverableReason: string | null }> }>()).then((data) => { setTemplates(data.templates); setUndeliverable(new Map(data.contacts.filter((contact) => contact.undeliverableAt).map((contact) => [contact.email, contact.undeliverableReason ?? '']))) }).catch(() => undefined)
  }, [])
  ...
  return <>
    ...
    >
      ...
      <div className={cn('flex min-h-0 flex-1 flex-col', minimized && 'hidden')}>
        ...
        {bounceReason !== undefined && <p role="status" className="flex shrink-0 items-start gap-2 border-t bg-amber-500/10 px-4 py-2 text-xs text-amber-700 dark:text-amber-300"><TriangleAlert className="mt-px size-3.5 shrink-0" /><span><Trans id="Earlier mail to this address could not be delivered." />{bounceReason && <span className="block break-words opacity-80">{bounceReason}</span>}</span></p>}
